...
export const Formats: import("../sim/dex-formats").FormatList = [
	...
	},
	{
		...
		onBegin() {
			...
			this.add(
				"-message",
				`This is a micrometa featuring just 6 Pokemon, each with 6 forms each!`
			);
			this.add(
				"-message",
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			if (
				...
				!set.moves.every((move) =>
					effectFunctions.some((f) => f.get(move).exists)
				)
			) {
				return this.validateSet(set, teamHas);
			}
			const allThings = [set.ability, set.item, ...set.moves].filter(
				(e) => e.length
			);
			for (const thing of allThings) {
				if (
					this.toID(thing) === "trace" ||
					this.toID(thing) === "neutralizinggas"
				) {
					return [`${thing} is currently bugged and is banned.`];
				}
				if (
					!dex.moves.get(thing).exists &&
					!dex.abilities.get(thing).exists &&
					!dex.items.get(thing).exists
				) {
			...
			if (
				allThings.some((y) =>
					effectFunctions.some(
						(x) =>
							x.get(y).isNonstandard &&
							!this.ruleTable.has(
								`+pokemontag:${this.toID(x.get(y).isNonstandard)}`
							)
					)
				)
			) {
			...
			if (
				requiredAbility &&
				!abilities.map((a) => a.id).includes(this.toID(requiredAbility))
			) {
			...
			if (
				requiredMove &&
				!moves.map((m) => m.id).includes(this.toID(requiredMove))
			) {
				return [`${set.species} requires ${requiredMove} on its set.`];
			}
			if (!items.length && requiredItems.length) {
				return [
					`${set.species} requires ${requiredItems.join(
						", "
					)} on its set.`,
				];
			}
			...
			if (items.length) {
				set.item = (
					items.find((i) => i.forcedForme || i.itemUser) || items[0]
				).id;
			} else {
				set.item = "";
			}
			...
			for (const curMove of moves) {
				...
				if (
					requiredMove &&
					moves.map((m) => m.id).includes(curMove.id) &&
				...
				if (problems)
					problems = problems.filter(
						(p) => !p.endsWith("needs to have an ability.")
					);
		...
		onBeforeSwitchIn(pokemon) {
			...
			if (
				pokemon.hasItem("abilityshield") ||
				pokemon.m.scrambled.items.some(
					(e: { thing: string }) => this.toID(e.thing) === "abilityshield"
				)
			) {
				ngas = false;
			}
			...
			if (ngas) {
				if (
					(pokemon.m.scrambled.moves as { inSlot: string }[]).findIndex(
						(e) => e.inSlot === "Ability"
					) >= 0
				) {
					const isMove = (
						pokemon.m.scrambled.moves as { inSlot: string }[]
					).findIndex((e) => e.inSlot === "Ability");
					const indexOfMove = pokemon.moveSlots.findIndex(
						(m) =>
							this.toID(pokemon.m.scrambled.moves[isMove].thing) === m.id
					);
		...
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				...
				if (this.dex.items.get(pokemon.set.ability).exists) {
					pokemon.m.scrambled.items.push({
						thing: this.dex.items.get(pokemon.set.ability).name,
						inSlot: "Ability",
					});
				} else if (this.dex.moves.get(pokemon.set.ability).exists) {
					pokemon.m.scrambled.moves.push({
						thing: this.dex.moves.get(pokemon.set.ability).name,
						inSlot: "Ability",
					});
				}

				if (this.dex.abilities.get(pokemon.set.item).exists) {
					pokemon.m.scrambled.abilities.push({
						thing: this.dex.abilities.get(pokemon.set.item).name,
						inSlot: "Item",
					});
				} else if (
					this.dex.moves.get(pokemon.set.item).exists &&
					this.dex.moves.get(pokemon.set.item).id !== "metronome"
				) {
					pokemon.m.scrambled.moves.push({
						thing: this.dex.moves.get(pokemon.set.item).name,
						inSlot: "Item",
					});
				}
				...
				for (const move of pokemon.set.moves) {
					...
					if (this.dex.abilities.get(move).exists) {
						pokemon.m.scrambled.abilities.push({
							thing: this.dex.abilities.get(move).name,
							inSlot: "Move",
						});
					} else if (this.dex.items.get(move).exists) {
						pokemon.m.scrambled.items.push({
							thing: this.dex.items.get(move).name,
							inSlot: "Move",
						});
					}
				}
				...
				for (const moveSlot of pokemon.baseMoveSlots) {
					if (moveSlot.id === "metronome") {
						...
						const cantMetronome = TeamValidator.get(
							this.format
						).checkCanLearn(
							this.dex.moves.get("metronome"),
							pokemon.species
						);
						if (!cantMetronome) {
							...
							pokemon.m.scrambled.items.push({
								thing: this.dex.items.get("metronome").name,
								inSlot: "Move",
							});
						}
						continue;
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			if (
				this.ruleTable.has("obtainablemoves") &&
				this.checkCanLearn(move, species, this.allSources(species), set)
			) {
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				const ability = this.dex.abilities.get(set.ability);
				if (
					this.ruleTable.isRestricted(`item:${item.id}`) ||
					this.ruleTable.isRestricted(`ability:${ability.id}`)
				) {
					return [`${species.name} is not allowed to hold ${item.name}.`];
				}
		...
		onSwitchIn(pokemon) {
			...
			if (
				...
				this.add(
					"-start",
					pokemon,
					originalSpecies.requiredItem || originalSpecies.requiredMove,
					"[silent]"
				);
				const oSpecies = this.dex.species.get(pokemon.m.originalSpecies);
				if (oSpecies.types.join("/") !== pokemon.species.types.join("/")) {
			...
			for (const set of team) {
				...
				if (
					(this.ruleTable.isRestrictedSpecies(species) ||
						...
						this.ruleTable.isRestricted(`ability:${ability.id}`)) &&
					!(
						((item.megaStone || item.isPrimalOrb) &&
							item.itemUser?.includes(species.baseSpecies)) ||
						(item.id === "ultranecroziumz" &&
							species.name.startsWith("Necrozma-"))
					)
				) {
	...
	},
	{
		...
		unbanlist: [
			...
			"Krookodile-Infernal",
			"Krookodile-Abyssal",
			"Krookodile-Wetlander",
			"Krookodile-Cave Dweller",
			"Krookodile-Cliffside",
			"Krookodile-Quantum",
			"Ogerpon-Muerta",
			"Ogerpon-Easter",
			"Ogerpon-Ankh",
	...
	},
	{
		...
		desc: `Randomized teams of Pok&eacute;mon for a generated Smogon tier with sets that are competitively viable.`,
		threads: [
			`&bullet; <a href="https://www.smogon.com/forums/threads/3775744/">1v1 Factory Set Discussion</a>`,
		],
		mod: "gen9",
