...
export class RuleTable extends Map<string, string> {
	...
	resolveNumbers(format: Format, dex: ModdedDex) {
		...
		if (
			...
			throw new Error(
				`Timer starting value ${timer.starting}${this.blame(
					"timerstarting"
				)} must be between 10 and 1200 seconds.`
			);
		}
		if (timer.grace && timer.grace > 300) {
			throw new Error(
				`Timer grace value ${timer.grace}${this.blame(
					"timergrace"
		...
		if (timer.addPerTurn && timer.addPerTurn > 30) {
			throw new Error(
				`Timer add per turn value ${timer.addPerTurn}${this.blame(
					"timeraddperturn"
		...
		if (
			...
			throw new Error(
				`Timer max per turn value ${timer.maxPerTurn}${this.blame(
					"timermaxperturn"
				)} must be between 10 and 1200 seconds.`
			);
		}
		if (
			...
			throw new Error(
				`Timer max first turn value ${timer.maxFirstTurn}${this.blame(
					"timermaxfirstturn"
				)} must be between 10 and 1200 seconds.`
			);
		}
...
export class DexFormats {
	...
	load(): this {
		...
		try {
			customFormats =
				require(`${__dirname}/../config/custom-formats`).Formats;
		...
		let Formats: AnyObject[] =
			require(`${__dirname}/../config/formats`).Formats;
	...
	validate(name: string) {
		...
		const customRules = customRulesString.split(",").map((rule) => {
			...
			if (typeof ruleSpec === "string") {
				if (
					ruleSpec === "-pokemontag:allpokemon" ||
					ruleSpec === "+pokemontag:allpokemon"
				) {
					if (hasPokemonRule)
						throw new Error(
							`You can't ban/unban pokemon before banning/unbanning all Pokemon.`
						);
				}
	...
	getRuleTable(
		...
		let skipPokemonBans = ruleSpecs.filter(
			(r) => r === "+pokemontag:allpokemon"
		).length;
		let hasPokemonBans = false;
		const warnForNoPokemonBans = !!skipPokemonBans && !format.customRules;
		skipPokemonBans += ruleSpecs.filter(
			(r) => r === "-pokemontag:allpokemon"
		).length;
		...
		for (let ruleSpec of ruleSpecs) {
			...
			if (ruleSpec.startsWith("!") && !ruleSpec.startsWith("!!")) {
				...
				if (repealDepth === undefined)
					throw new Error(
						`Multiple "${ruleSpec}" rules in ${format.name}`
					);
				if (repealDepth === depth && !noWarn) {
					throw new Error(
						`Rule "${ruleSpec}" did nothing because "${ruleSpec.slice(
							1
						)}" is not in effect`
					);
				}
			...
			if ("+*-".includes(ruleSpec.charAt(0))) {
				if (ruleTable.has(ruleSpec)) {
					throw new Error(
						`Rule "${ruleSpec}" in "${format.name}" already exists in "${
